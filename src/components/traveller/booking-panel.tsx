...
export function BookingPanel({
    ...
    const bidData : BidData = {
        ...
        check_out: selectedDates.length > 0
    ...
    const {setPropertyId} = useWinner()
    useEffect(() => {
        setPropertyId(property_id)
    }, [property_id, setPropertyId])

    const { user } = useAuth();
    const currentUserName = user?.name ? user.name.toString() : "Know user";
                    ...
                    <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    #{user_id.toString().slice(0,8)}
                                    {user_id.toString().length > 8 ? "..." : "" }
                                </CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                                    ...
                                    {bidPerNight > 0 && (
                                        <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                                            <div className="text-center">
                                                <div className="text-green-800 text-sm">Per day</div>
